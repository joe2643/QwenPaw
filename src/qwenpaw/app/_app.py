...
async def lifespan(  # pylint: disable=too-many-statements,too-many-branches
    ...
    # Media server starts in _background_startup; placeholder so endpoints
    # that inspect app.state.media_server before the background task runs
    # don't AttributeError.
    app.state.media_server = None
    ...
    async def _background_startup():  # pylint: disable=too-many-statements
        try:
            ...
            # --- Global media server (process-level, shared across agents) ---
            try:
                config_ms = load_config(get_config_path())
                if config_ms.media_server.enabled:
                    from .media_server import MediaServer
                    from urllib.parse import urlparse

                    ms_cfg = config_ms.media_server
                ...
                logger.error(
                    "Failed to start media server: %s",
                    _ms_exc,
                    exc_info=True,
                )
