...
class SignalDaemon:
    ...
    def _to_recipient(target: str, is_group: bool) -> str:
        ...
        `group.xxx` string, or QwenPaw's session-prefixed `group:xxx`
        form (from effective_sender). Returns bbernhard-compatible
        recipient.
        """
        if not is_group:
            return target
        # Strip QwenPaw session prefix so we don't double-wrap.
...
class SignalChannel(BaseChannel):
    ...
    async def _stream_with_tracker(self, payload):
        """Override base to handle QwenPaw event format for Signal."""
