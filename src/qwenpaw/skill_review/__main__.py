# -*- coding: utf-8 -*-
"""CLI entry point for qwenpaw.skill_review.

Usage:
    python -m qwenpaw.skill_review --agent default
    python -m qwenpaw.skill_review --agent default --workspace ~/.qwenpaw/workspaces/default
    python -m qwenpaw.skill_review --agent default --dry-run
    python -m qwenpaw.skill_review --agent discord-triage --agent ai-news
"""
import argparse
import logging
import sys
...
def _default_workspace(agent: str) -> Path:
    # TODO: adjust if QwenPaw workspace root differs from ~/.qwenpaw/workspaces/
    return Path.home() / ".qwenpaw" / "workspaces" / agent


def main() -> None:
    parser = argparse.ArgumentParser(
        prog="python -m qwenpaw.skill_review",
        description="QwenPaw Skill Review — offline WAL-based skill creation",
    )
    parser.add_argument(
        "--agent",
        required=True,
