...
def _ensure_skills_manager_importable() -> None:
    """Register a stub for qwenpaw.agents.skills_manager if not importable.

    patch("qwenpaw.agents.skills_manager.SkillService") requires the module to
    already be in sys.modules (because qwenpaw.agents.__init__ uses a lazy
    ...
    """
    key = "qwenpaw.agents.skills_manager"
    if key in sys.modules:
        return
    try:
        import qwenpaw.agents.skills_manager  # noqa: F401
