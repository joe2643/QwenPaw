...
class TestResolveMediaSource:
    ...
    @patch("qwenpaw.agents.tools.view_media._get_signed_url", new_callable=AsyncMock, return_value="https://media.example.com/signed")
    @pytest.mark.asyncio
    ...
    @patch("qwenpaw.agents.tools.view_media._get_signed_url", new_callable=AsyncMock, return_value=None)
    @pytest.mark.asyncio
...
class TestViewImage:
    ...
    async def test_valid_image_returns_image_block(self, tmp_image):
        with patch("qwenpaw.agents.tools.view_media._get_signed_url", new_callable=AsyncMock, return_value=None):
    ...
    async def test_signed_url_used_when_available(self, tmp_image):
        with patch("qwenpaw.agents.tools.view_media._get_signed_url", new_callable=AsyncMock, return_value="https://media.example.com/img"):
            result = await view_image(str(tmp_image))
...
class TestViewVideo:
    ...
    async def test_signed_url_returns_video_block(self, tmp_video):
        with patch("qwenpaw.agents.tools.view_media._get_signed_url", new_callable=AsyncMock, return_value="https://media.example.com/vid"):
    ...
    async def test_no_server_extracts_keyframes(self, tmp_path):
        ...
        with patch("qwenpaw.agents.tools.view_media._get_signed_url", new_callable=AsyncMock, return_value=None), \
             patch("qwenpaw.agents.tools.view_media._model_supports_video", return_value=False):
    ...
    async def test_no_server_no_ffmpeg_returns_error(self, tmp_video):
        ...
        with patch("qwenpaw.agents.tools.view_media._get_signed_url", new_callable=AsyncMock, return_value=None), \
             patch("qwenpaw.agents.tools.view_media._model_supports_video", return_value=False), \
             patch("qwenpaw.agents.tools.view_media._extract_keyframes", return_value=[]):
                result = await view_video(str(tmp_video))
...
class TestModelCapabilityCheck:
    ...
    async def test_video_unsupported_model_uses_keyframes(self, tmp_path):
        ...
        with patch("qwenpaw.agents.tools.view_media._model_supports_video", return_value=False):
            result = await view_video(str(video))
            types = [b.get("type") for b in result.content if isinstance(b, dict)]
    ...
    async def test_video_supported_model_sends_video(self, tmp_video):
        ...
        with patch("qwenpaw.agents.tools.view_media._model_supports_video", return_value=True):
            with patch("qwenpaw.agents.tools.view_media._get_signed_url", new_callable=AsyncMock, return_value="https://media.example.com/v"):
                result = await view_video(str(tmp_video))
                types = [b.get("type") for b in result.content if isinstance(b, dict)]
                assert "video" in types
    ...
    async def test_video_unsupported_no_ffmpeg_returns_error(self, tmp_video):
        ...
        with patch("qwenpaw.agents.tools.view_media._model_supports_video", return_value=False):
            with patch("qwenpaw.agents.tools.view_media._extract_keyframes", return_value=[]):
                result = await view_video(str(tmp_video))
                text = " ".join(b.get("text", "") for b in result.content if isinstance(b, dict))
                assert "does not support video" in text
                types = [b.get("type") for b in result.content if isinstance(b, dict)]
...
class TestMediaConfigGlobalSecret:
    ...
    def test_uses_runtime_secret_when_config_empty(self):
        ...
        from qwenpaw.agents.tools.view_media import _get_media_config
        from qwenpaw.app import media_server as ms_mod

        old = ms_mod._runtime_secret
        ms_mod._runtime_secret = "global-runtime-secret"

        with patch("qwenpaw.config.utils.load_config", side_effect=Exception("no config")):
    ...
    def test_config_secret_takes_precedence(self):
        ...
        from qwenpaw.agents.tools.view_media import _get_media_config
        from qwenpaw.app import media_server as ms_mod
        from qwenpaw.config.config import MediaServerConfig
        ...
        with patch("qwenpaw.config.utils.load_config", return_value=mock_config):
            cfg = _get_media_config()
